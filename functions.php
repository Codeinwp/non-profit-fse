 ...
 */
function bootstrap() {
	global $_nonprofit_fse_bootstrap_errors;

	$_nonprofit_fse_bootstrap_errors = new \WP_Error();
...
function check_php() {
	...
	$_nonprofit_fse_bootstrap_errors->add(
		'php_version',
		sprintf(
		/* translators: %s message to upgrade PHP to the latest version */
			...
			/* translators: %s message to upgrade PHP to the latest version */
				...
				__( 'upgrading PHP to the latest version', 'nonprofit-fse' )
			),
			'7.0'
		)
	);
...
function check_build_files() {
	if ( defined( 'NONPROFIT_FSE_IGNORE_SOURCE_CHECK' ) ) {
		return;
	}

	$_files_to_check = array(
		NONPROFIT_FSE_DIR . 'vendor/autoload.php',
		NONPROFIT_FSE_DIR . 'assets/css/build/style.css',
		NONPROFIT_FSE_DIR . 'assets/css/build/editor.css',
		NONPROFIT_FSE_DIR . 'assets/css/build/style-rtl.css',
		NONPROFIT_FSE_DIR . 'assets/css/build/editor-rtl.css',
	);
...
function maybe_add_notices() {
	...
	if ( ! $_nonprofit_fse_bootstrap_errors->has_errors() ) {
		return;
	}
	...
	add_action(
		...
		function () {
			...
			printf( '<div class="notice notice-error"><p>%1$s</p></div>', wp_kses_post( $_nonprofit_fse_bootstrap_errors->get_error_message() ) );
		}
	);
}
...
function load_sdk() {
	add_filter(
		...
		function ( $products ) {
			$products[] = NONPROFIT_FSE_DIR . 'style.css';
...
function load_dependencies() {
	$vendor_file = NONPROFIT_FSE_DIR . '/vendor/autoload.php';
